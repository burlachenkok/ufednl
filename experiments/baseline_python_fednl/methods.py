...
class Standard_Newton:
    ...
    def find_optimum(self, x0, n_steps=10, verbose=True):
        ...
        for k in range(n_steps):
            ...
            print('-- Newton step took time {:.3f} seconds'.format(time2 - time1), flush=True)
...
class FedNL:
    ...
    def method(self, x, hes_comp_param, hes_comp_name='LowRank', init='zero',\
        ...
        #while func_value[-1] > f_opt + tol and n_iters <= max_iter:
            ...
            if verbose:
                print(func_value[-1], time.time() - startTm, "seconds (per round)", flush=True)
...
class FedNL_CR:
    ...
    def method(self, x, hes_comp_param, hes_comp_name='LowRank', init='zero', init_cost=True, upd_rule=1, max_iter=100, tol=1e-15,\
        ...
        if verbose:
            print(func_value[-1], (time.time()-beginTm), " seconds (init)", flush=True)
        ...
        while func_value[-1] > f_opt + tol and n_iters <= max_iter:
            ...
            if verbose:
                print(func_value[-1], (time.time() - beginTm), " seconds (per step)", flush=True)
