...
role="server"
client_number=0
bin="./../../../../dopt/scripts/build_release/bin_fednl_distr_client_pp/bin_fednl_distr_client_pp"

if ((SLURM_PROCID == 0)); then
  role="server"
  bin="./../../../../dopt/scripts/build_release/bin_fednl_distr_master_pp/bin_fednl_distr_master_pp"
else
...
${bin} --line_search --c_line_search 0.49 --gamma_line_search 0.5 --server_grad_workers 0 --server_hessian_workers 0 --k_compressor_as_d_mult 8 --algorithm fednl1 --train --train_dataset ${dataset} --add_intercept --reshuffle_train --clients ${nclients} --lambda 0.001 --fednl-option-b --rounds ${rounds} --tracking --check_split --compressor ${compressor} --theoretical_alpha --use_theoretical_alpha_option_2 --out "result_run_{run_id}_{algorithm}_{dataset}_m{kmultiplier}_${compressor}_option_2.bin" --master tcpv4:${master_host}:${master_port_base} --iam ${role} --clients-per-round ${clientsperround} ${extra_flags} 1 >> $log
#===================================================================================================
