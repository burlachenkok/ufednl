...
int physicalProcessors(int * logical_processors) {
    ...
    bool hyperthreadingSupported = false;        // CPU supports hyper-threading / simultaneous multi-threading
    ...
    if (vendor == 1) {
        ...
        int hyper_ = 0;                          // hyper-threading status: 0 = unknown, 1 = disabled, 2 = enabled
        if (maxLeaf >= 0xB) {                    // leaf 0xB or 0x1F: Extended Topology Enumeration
            ...
            // The number of performance monitor registers depends on hyper-threading status
            ...
            if (perfVersion == 3 || perfVersion == 4) {
                if (perfNum == 4) {
                    hyper_ = 2;                   // 4 performance registers when hyper-threading enabled
                }
                else if (perfNum == 8) {         // 8 performance registers when hyper-threading disabled
                    hyper_ = 1;
                    procPerCore = 1;
                    logicalProc = physicalProc;  // reduce the number of logical processors when hyper-threading is disabled
                }
            ...
            if (hyperthreadingSupported) {
                // number of logical processors per core is not known. Assume 2 if hyper-threading supported
                procPerCore = 2;
            }
            physicalProc = logicalProc / procPerCore;
        }
        ...
        if (systemProcessors > logicalProc) {
            // Multiple CPU chips. Assume that chips are identical with respect to hype-threading
            physicalProc = systemProcessors * physicalProc / logicalProc;
            logicalProc = systemProcessors;
        }
        else if (logicalProc > systemProcessors && systemProcessors > 0 && hyper_ == 0) {
        ...
        if (maxLeaf8 >= 8) {
            ...
            if (maxLeaf8 >= 0x1E) {
                ...
                // procPerCore = 2 if simultaneous multi-threading is enabled, 1 if disabled
            }
            else {
                if (hyperthreadingSupported) {
                    procPerCore = 2;
