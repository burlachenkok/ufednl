...
template<>
inline bool dopt::MatrixNMD<dopt::VectorNDRaw_f>::hasSIMDSupport() {
    return true;
}
...
namespace dopt
{
    //
    template<>
    inline bool MatrixNMD<VectorNDStd_f>::hasSIMDSupport() {
        return true;
    }
    //

    template<>
    inline MatrixNMD<VectorNDRaw_f>::MatrixColumn MatrixNMD<VectorNDRaw_f>::matrixVectorMultiply(const MatrixNMD<VectorNDRaw_f>& m,
                                                                                                 const MatrixNMD<VectorNDRaw_f>::MatrixColumn& x)
    {
        assert(m.columns() == x.size());

        typedef typename VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
        constexpr size_t kVecBatchSize = getVecBatchSize<VecType>();
        ...
        size_t packed_items_in_column = roundToNearestMultipleDown<kVecBatchSize>(r);
        ...
        size_t j = 0;
        size_t cBound =  roundToNearestMultipleDown<kColumnsToProcess>(c);
        size_t myLDA = m.LDA;
    ...
    template<>
    inline MatrixNMD<VectorNDStd_f>::MatrixColumn MatrixNMD<VectorNDStd_f>::matrixVectorMultiply(const MatrixNMD<VectorNDStd_f>& m,
                                                                                                 const MatrixNMD<VectorNDStd_f>::MatrixColumn& x)
    {
        assert(m.columns() == x.size());
...
//=============================================================================================================//
    ...
    inline void transpose4x4_SSE_variant_a(float* restrict_ext B, const size_t ldb, const float* restrict_ext A, const size_t lda)
    {
        // For support this code it's enough to have SSE2 support.
        __m128 row0 = _mm_loadu_ps(&A[0 * lda]);
        __m128 row1 = _mm_loadu_ps(&A[1 * lda]);
        __m128 row2 = _mm_loadu_ps(&A[2 * lda]);
        __m128 row3 = _mm_loadu_ps(&A[3 * lda]);
        {
            // Implementation from Microsoft MSVC 14.32.31326 (include/xmmintrin.h)
        ...
        _mm_storeu_ps(&B[0 * ldb], row0);
        _mm_storeu_ps(&B[1 * ldb], row1);
        _mm_storeu_ps(&B[2 * ldb], row2);
        _mm_storeu_ps(&B[3 * ldb], row3);
    }
#endif

    template<>
    inline void MatrixNMD<dopt::VectorNDRaw_f>::internal_execute_transpose_blockwise(float* restrict_ext B, const size_t ldb,
                                                                                     const float* restrict_ext A, const size_t lda,
...
#if DOPT_INCLUDE_VECTORIZED_CPU_TRANSPOSE_MATS && (SUPPORT_CPU_SSE2_128_bits || SUPPORT_CPU_AVX_256_bits || SUPPORT_CPU_AVX_512_bits)
    ...
    template<>
    inline void MatrixNMD<dopt::VectorNDStd_f>::internal_execute_transpose_blockwise(float* restrict_ext B, const size_t ldb,
                                                                                     const float* restrict_ext A, const size_t lda,
                                                                                     const size_t r, const size_t c)
    {
        // B matrix contains "B" by columns. Stride between columns is LDB
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDRaw_f> MatrixNMD<dopt::VectorNDRaw_f>::getTranspose() const
    {
        size_t r = rows();
        size_t c = columns();
        MatrixNMD res(c, r);
        MatrixNMD<dopt::VectorNDRaw_f>::internal_execute_transpose_blockwise(res.matrixByCols.data(), res.LDA, matrixByCols.dataConst(), LDA, r, c);
        return res;
    }

    template<>
    inline MatrixNMD<dopt::VectorNDStd_f> MatrixNMD<dopt::VectorNDStd_f>::getTranspose() const
    {
        size_t r = rows();
        size_t c = columns();
        MatrixNMD res(c, r);
        MatrixNMD<dopt::VectorNDStd_f>::internal_execute_transpose_blockwise(res.matrixByCols.data(), res.LDA, matrixByCols.dataConst(), LDA, r, c);
        return res;
    }

    template<>
    inline MatrixNMD<dopt::VectorNDStd_f>::TElementType MatrixNMD<dopt::VectorNDStd_f>::frobeniusNormSquareForSymmetricMatrixFromUpPart() const
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDStd_f>::TElementType MatrixNMD<dopt::VectorNDStd_f>::frobeniusNormForSymmetricMatrixFromUpPart() const
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDRaw_f>::TElementType MatrixNMD<dopt::VectorNDRaw_f>::frobeniusNormSquareForSymmetricMatrixFromUpPart() const
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDRaw_f>::TElementType MatrixNMD<dopt::VectorNDRaw_f>::frobeniusNormForSymmetricMatrixFromUpPart() const
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDRaw_f>::MatrixColumn MatrixNMD<dopt::VectorNDRaw_f>::matrixVectorMultiplyWithPreTranspose(const MatrixNMD<dopt::VectorNDRaw_f>& mTranspose,
                                                                                                                             const MatrixNMD<dopt::VectorNDRaw_f>::MatrixColumn& x)
    {
    ...
    template<>
    inline MatrixNMD<dopt::VectorNDStd_f>::MatrixColumn MatrixNMD<dopt::VectorNDStd_f>::matrixVectorMultiplyWithPreTranspose(const MatrixNMD<dopt::VectorNDStd_f>& mTranspose,
                                                                                                                             const MatrixNMD<dopt::VectorNDStd_f>::MatrixColumn& x)
    {
        assert(mTranspose.rows() == x.size());
    ...
    template<>
    inline void MatrixNMD<dopt::VectorNDRaw_f>::applyNaturalCompressor()
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
        constexpr size_t kVecBatchSize = dopt::getVecBatchSize<VecType>();
        constexpr size_t kUnrollFactor = dopt::getUnrollFactor<VecType>();
    ...
    template<>
    inline void MatrixNMD<dopt::VectorNDStd_f>::applyNaturalCompressor()
    {
        typedef dopt::VectorSimdTraits<TElementType, cpu_extension>::VecType VecType;
        constexpr size_t kVecBatchSize = dopt::getVecBatchSize<VecType>();
        constexpr size_t kUnrollFactor = dopt::getUnrollFactor<VecType>();
