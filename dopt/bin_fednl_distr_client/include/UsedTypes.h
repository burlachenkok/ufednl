...
{
#if D_OPT_USE_ATOMICS_IN_DISTRIB_IMPL
    std::atomic<bool> gradIsReady = false;  ///< Signal setuped by worker when result is ready
    std::atomic<bool> fiIsReady = false;    ///< Signal setuped by worker when result is ready
#else
    bool gradIsReady = false;               ///< Signal setuped by worker when result is ready
    bool fiIsReady = false;                 ///< Signal setuped by worker when result is ready
#endif
};
...
struct WorkerContextForGD final: public WorkerContext
{
    ...
    OptimizationProblem* optProblem = nullptr;  ///< Optimization Problem
    int workerIndex = -1;                       ///< Index of worker
    VectorND_d localGradient;                   ///< Result from last iteration (direction)
    GDControlBlock ctrBlock;                    ///< Control Block

    bool send_fi_from_worker = false;           ///< Flag denoted to fact that worker should compute fi and fill messageToSendFi.
    
    VectorND_d::TElementType fiValue = VectorND_d::TElementType(); ///< Function value in current iterate
