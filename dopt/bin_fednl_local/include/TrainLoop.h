...
#include "dopt/math_routines/include/ConvexOptimizationRoutines.h"

// Threadpool
#include "dopt/system/include/threads/ThreadPoolWithTaskQueue.h"

// Callbacks
#include "TrainCallbaksShared.h"
...
inline void waitForInformationFromClientsForFedNL(const std::vector<WorkerContext*>& localWorkersContext,  bool is_algorithm_fednl_has_option_b)
{
    ...
    {
        for (WorkerContextForFedNL* ctx = (WorkerContextForFedNL*)localWorkersContext[c]; ctx->ctrBlock.messageToSendGradientsIsReady != true;) {
    ...
    for (size_t c = 0; c < nClients; ++c)
    {
        for (WorkerContextForFedNL* ctx = (WorkerContextForFedNL*)localWorkersContext[c]; ctx->ctrBlock.messageToSendHessiansIsReady != true;) {
            dopt::DefaultThread::yeildCurrentThInHotLoop();
        }
    }
    ...
    if (is_algorithm_fednl_has_option_b)
    {
        ...
        {
            for (WorkerContextForFedNL* ctx = (WorkerContextForFedNL*)localWorkersContext[c]; ctx->ctrBlock.messageToSendLkIsReady != true;) {
                dopt::DefaultThread::yeildCurrentThInHotLoop();
            }
        }
    }
...
inline TrainReturnCodes train(const dopt::CmdLine& cmdline, ResultCallbackLocal resultCallback)
{
    ...
    for (int c = 0; c < nClients; ++c)
    {
        ...
        else if (is_algorithm_fednl1)
        {
            ...
            //std::cout << ">>" << ctx->learningHessian.frobeniusNormForSymmetricMatrixFromUpPart() << "\n";
    ...
    if (args.tracking.tracking_is_on)
    {
        ...
        if (is_algorithm_fednl1)
        {
            trackingData.putString("FedNL", dopt::MutableData::PutStringFlags::ePutNoTerminator);
            if (has_line_search)
                trackingData.putString("-LS", dopt::MutableData::PutStringFlags::ePutNoTerminator);
        ...
        else if (used_compressor == Compressor::eNatural)
        {
            ...
            for (size_t j = 0; j < d; ++j)
            {
                ...
                for (size_t i = 0; i <= j; ++i)
                {
                    ...
                    {
                        size_t szBytes = messageHessianItems.getBytes(&buffer, 3);
                        assert(szBytes == 3);
    ...
    
    {
        ...
        for (int r = 0; r < rounds; ++r)
        {
            ...
            if (is_algorithm_fednl1)
            {
                ...
                // Start new round. Only master can start new 0 round. Next rounds are started in the end.
                if (r == 0)
                {
#if DOPT_USE_SEMPAHORE_DURING_SYNC_IN_CLIENTS
                    serverContext.startSempahore.release(nWorkers);
                ...
                // Compute norm of current iterate
                xCurL2Norm = xCur.vectorL2Norm();
                ...
                int waitlist_with_grad_info = nClients;
                int waitlist_with_shifted_hessian_info = nClients;
                int waitlist_with_Lk_info = is_algorithm_fednl_has_option_b ? nClients : -1;

                //===============================================================================
                // Debug mode: wait for information from all clients: grad, hessians, LK
                //===============================================================================
                if (debugFlags.debugForceSequentialUpdate)
                    waitForInformationFromClientsForFedNL(localWorkersContext, is_algorithm_fednl_has_option_b);
                ...
                if (is_algorithm_fednl_has_option_a)
                {
                    ...
                    if (algorithmNumberForSystem == 1)
                    {
                        ...
                        dx = dopt::gausEleminationSolver(matrixOfSlau, global_gradient_estimation);

                        if constexpr (kDebugOutputForSolve)
                        {
                            if (printAtTraining)
                                std::cout << "SOLVE DISCREPANCY:" << (matrixOfSlau * dx - global_gradient_estimation).vectorL2Norm() << "!!\n";
                        }
                    }
                    else if (algorithmNumberForSystem == 2)
                    {
                        ...
                        if (sstFactorization)
                        {
                            ...
                            dx = dopt::backwardSubstitutionWithATranspose(chol_l_factor_tr, Ltr_dx);
                ...
                else
                {
                    ...
                    dx = used_global_step_size * global_gradient_estimation;
                }
            }
            ...
            if (args.tracking.tracking_is_on || args.optAlgo.has_line_search)
            {
                int waitlist_with_function_info = nClients;
            ...
            //===============================================================================================
            // Update iterate (line search process)
            size_t kLineSearchIteration = 0;
            
            if (has_line_search)
            {
                //===========================================================================================
                // Start line search
                // t is parameter playing role of \gamma^s from FedNL-NS
                double t = t_start_line_search;

                double f_xk = function_aggregation;

                // (-dx) plays a role of "d^k" from FedNL-NS
                double dx_dot_gradient = -(dx & global_gradient_estimation);

                const size_t kMaxIerations = args.optAlgo.max_line_search_steps;

                VectorND_d xCurPrev = xCur;

                for (kLineSearchIteration = 0; kLineSearchIteration < kMaxIerations;)
                {
                    // Update value of iterate
                    xCur = xCurPrev - (t * dx);

                    // Allow to start line search
                    serverContext.lineSearchIteration = kLineSearchIteration;

                    // Construct function_aggregation_request
                    VectorND_d::TElementType function_aggregation_request = VectorND_d::TElementType();
                    {
                        int waitlist_with_function_info = nClients;
                        ...
                        while (waitlist_with_function_info >= 0)
                        {
                            for (int c = 0; c < nClients; ++c)
                            {
                                ...
                                if (dopt::checkAndResetIfSet(&ctx->ctrBlock.messageToSendFiIsReady))
                                {
                                    ...
                                    // Unpack the results: start
                                    dopt::Data message = dopt::Data(ctx->messageToSendFi.getPtr(), ctx->messageToSendFi.getFilledSize(), dopt::Data::MemInitializedType::eGiftWholeMemoryPleaseNotFree);
                                    double fi = message.getDouble();
                                    function_aggregation_request += fi;
                                    receivedBytesFromClientsForScalarsLast += ctx->messageToSendFi.getFilledSize();
                                }
                            }
                            ...
                            {
                                // All workers report their gradient information
                                function_aggregation_request *= inv_NClients;
                                waitlist_with_function_info -= 1;
                            }
                        }
                    }
                    ...
                    if (function_aggregation_request <= function_aggregation + (c_line_search * t) * dx_dot_gradient)
                    {
                        // Line search condition is satisfied. Finish.
                        serverContext.lineSearchIteration = std::numeric_limits<size_t>::max();
                        serverContext.lineSearchRound = size_t(r + 1);

                        // Allow to start LS iteration with a very big "k" - i.e. turn off temporaly LS iteration via busy-loop in client
                        // Allow start LS in a next round - i.e. clients can terminate current round                                                                                                  // However, previous setup dissallow start iterations until "serverContext.lineSearchIteration = 0"
                    ...
                    {
                        // Line search condition is not satisfied
                        t *= gamma_line_search;                        
                        ++kLineSearchIteration;
                    }
                }

                if (kLineSearchIteration == kMaxIerations)
                {
                    // Force termination for Line-Search
                    serverContext.lineSearchIteration = std::numeric_limits<size_t>::max();
                    serverContext.lineSearchRound = size_t(r + 1);

                    //serverContext.lineSearchIteration = std::numeric_limits<size_t>::max();
                    // Allow to start LS iteration with a very big "k" - i.e. turn off temporaly LS iteration via busy-loop in client
                    //serverContext.lineSearchRound = r + 1;
                    // Allow start LS in a next round - i.e. clients can terminate current round
                    //dopt::internal::memoryFence();
                }
                //===========================================================================================
            }
            //===============================================================================================
            // Early start of next round
            if (r + 1 < rounds)
            {
                // allow next round to start
#if DOPT_USE_SEMPAHORE_DURING_SYNC_IN_CLIENTS
                serverContext.startSempahore.release(nWorkers);
#endif
                
                // Implicit memory syncronization
                serverContext.roundToStart = size_t(r + 1);
            }
            ...
            if (!silentRun && printAtTraining)
            {
                ...
                if (args.tracking.tracking_is_on || args.optAlgo.has_line_search)
                {
                    std::cout << ": f(x): " << function_aggregation;
                }
                else
                {
                    std::cout << ": f(x): " << "[N/A]";
                ...
                std::cout << ": avg. time per round: " << timer.getTimeMs() / double(r + 1) << " msec"
                          << ": rounds/second: " << double(r + 1) / timer.getTimeSec()
                          << ": line-search num: " << kLineSearchIteration
                          << '\n';
            ...
            if (args.tracking.tracking_is_on)
            {
                ...
                trackingData.putDouble(global_gradient_estimation.vectorL2Norm());
    ...
    auto gradNormSqr = global_gradient_estimation.vectorL2NormSquare();

    if (printAtTraining) {
        std::cout << "  Norm of full gradient in last iterate: " << std::sqrt(gradNormSqr) << '\n';
        std::cout << "  Norm square of full gradient in last iterate: " << gradNormSqr << '\n';
        std::cout << "  Suboptimality gap of Function, f(x) - f*: " << dopt::functionValueSuboptimalityGap(gradNormSqr, mathMuf) << '\n';
        std::cout << "  Suboptimality distance to Optimal Solution |x - x*|: " << dopt::distanceSuboptimalityGap(gradNormSqr, mathMuf) << '\n';
        std::cout << "=========================================================================\n";
    ...
    if (resultCallback) {
        resultCallback(xCur.size(),
                       std::sqrt(gradNormSqr),
                       dopt::functionValueSuboptimalityGap(gradNormSqr, mathMuf),
                       dopt::distanceSuboptimalityGap(gradNormSqr, mathMuf)
                       );
    }
