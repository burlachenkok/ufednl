...
struct OptAlgorithmCfg
{
    ...
    //============================================================================================
    bool has_line_search = false;                          ///< Use line search to polish direction multiplier
    
    double c_line_search = 0.5;                            ///< Line search parameter c in (0, 0.5)

    double gamma_line_search = 0.5;                        ///< Line search parameter alpha in (0, 1)    

    uint32_t max_line_search_steps = 5;                    ///< Maximum number of line search steps
    //============================================================================================
};
...
inline TrainReturnCodes parseFlagsForOptAlgortihm(OptAlgorithmCfg& optAlgoCfg, const dopt::CmdLine& cmdline)
{
    ...
    if (optAlgoCfg.algorithm != "gd")
    {
        ...
        optAlgoCfg.has_line_search = cmdline.isFlagSetuped("line_search");

        if (optAlgoCfg.has_line_search)
        {
            if (!cmdline.getDoubleArgByName(optAlgoCfg.c_line_search, "c_line_search"))
            {
                std::cerr << "Please specify argument \"" << "c_line_search" << "\" for line search. Line search parameter c in (0, 0.5). \n";
                return TrainReturnCodes::eMissingArgument;
            }

            if (!cmdline.getDoubleArgByName(optAlgoCfg.gamma_line_search, "gamma_line_search"))
            {
                std::cerr << "Please specify argument \"" << "gamma_line_search" << "\" for line search. Line search parameter c in (0, 1). \n";
                return TrainReturnCodes::eMissingArgument;
            }

            cmdline.getUnsignedArgByName(optAlgoCfg.max_line_search_steps, "max_line_search_steps");
        }
    }
    else
    {
