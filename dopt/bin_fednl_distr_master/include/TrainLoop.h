...
// Callbacks
#include "TrainCallbaksSharedMaster.h"
                                                  ...
                                                  bool is_algorithm_fednl_has_option_b)
{
    ...
    for (size_t c = 0; c < nClients; ++c)
    {
        for (;;)
        {
            ...
            if (ctx->ctrBlock.messageToSendGradientsIsReady)
            {
                break;
            }
            else
            {
                ctx->receiveUpdates</*block*/true> (ctx->control);
            }
        }
    }
    ...
    if (is_algorithm_fednl_has_option_b)
    {
        for (int c = 0; c < nClients; ++c)
        {
            ...
            for (;;)
            {
                ...
                if (ctx->ctrBlock.messageToSendLkIsReady)
                {
                    break;
                }
                else
...
inline TrainReturnCodes train(const dopt::CmdLine& cmdline, ResultCallbackMaster resultCallback)
{
    ...
    if (is_algorithm_fednl1)
    {
        ...
        for (auto i = workersConnections[kControl].begin(); i != workersConnections[kControl].end(); ++i)
        {
            ...
            H += hessian_local;
            //std::cout << ">>" << hessian_local.frobeniusNormForSymmetricMatrixFromUpPart() << "\n";
        }
    ...
    {
        dopt::MutableData bufferForLSUpdates;          ///< Memory buffer for line-search updates
        dopt::MutableData bufferForRoundAndIterate;    ///< Memory buffer for sending round and iterate to clients
        ...
        for (int r = 0; r < rounds; ++r)
        {
            if (is_algorithm_gd)
            {
                ...
                bufferForRoundAndIterate.rewindToStart();
                serverContext.prepareUpdate(bufferForRoundAndIterate, ServerContext::sig_update_round_and_iterate);
                
                // Send update to all clients
                for (auto i = workersConnections[kControl].begin(); i != workersConnections[kControl].end(); ++i)
                {
                    dopt::Socket& socket = *(i->second);
                    bool result = socket.sendData(bufferForRoundAndIterate.getPtr(), bufferForRoundAndIterate.getFilledSize());
                    assert(result == true);
                }
            ...
            if (is_algorithm_fednl1)
            {
                ...
                // Inform clients [at zero iteration it happens here, at next iterations it happens ASAP as iterate is ready at the end of the loop]
                if (r == 0)
                {
                    bufferForRoundAndIterate.rewindToStart();
                    serverContext.prepareUpdate(bufferForRoundAndIterate, ServerContext::sig_update_round_and_iterate);
            ...
            if (has_line_search)
            {
                ...
                for (kLineSearchIteration = 0; kLineSearchIteration < kMaxIerations;)
                {
                    ...
                    {
                        bufferForLSUpdates.rewindToStart();
                        serverContext.prepareUpdate(bufferForLSUpdates, ServerContext::sig_update_linesearch_round_and_linesearch_iteration_and_iterate);
                        for (auto i = workersConnections[kControl].begin(); i != workersConnections[kControl].end(); ++i)
                        {
                            bool result = i->second->sendData(bufferForLSUpdates.getPtr(), bufferForLSUpdates.getFilledSize());
                            assert(result == true);
                        }
                    }
                    ...
                    VectorND_d::TElementType function_aggregation_request = VectorND_d::TElementType();
                    {
                        ...
                        while (waitlist_with_function_info >= 0)
                        {
                            for (int c = 0; c < nClients; ++c)
                            {
                                ...
                                {
                                    waitlist_with_function_info -= 1;
                                    function_aggregation_request += ctx->messageToSendFi;
                                    receivedBytesFromClientsForScalarsLast += sizeof(ctx->messageToSendFi);
                                }
                            }
                    ...
                    if (function_aggregation_request <= function_aggregation + (c_line_search * t) * dx_dot_gradient)
                    {
                        ...
                        serverContext.lineSearchIteration = std::numeric_limits<size_t>::max();   // Allow to start LS iteration with a very big "k" - i.e. turn off temporaly LS iteration via busy-loop in client
                        serverContext.lineSearchRound = size_t(r + 1);                            // Allow start LS in a next round - i.e. clients can terminate current round
                                                                                                  // However, previous setup dissallow start iterations until "serverContext.lineSearchIteration = 0"

                        // Update iteration and round
                        {
                            bufferForLSUpdates.rewindToStart();
                            serverContext.prepareUpdate(bufferForLSUpdates, ServerContext::sig_update_linesearch_round_and_linesearch_iteration);
                            for (auto i = workersConnections[kControl].begin(); i != workersConnections[kControl].end(); ++i)
                            {
                                bool result = i->second->sendData(bufferForLSUpdates.getPtr(), bufferForLSUpdates.getFilledSize());
                                assert(result == true);
            ...
            if (r + 1 < rounds)
            {
                ...
                for (auto i = workersConnections[kControl].begin(); i != workersConnections[kControl].end(); ++i)
                {
                    bool result = i->second->sendData(bufferForRoundAndIterate.getPtr(), bufferForRoundAndIterate.getFilledSize());
                    assert(result == true);
                }
            }
    ...
    if (printAtTraining)
    {
        ...
        std::cout << "  Norm of full gradient in last iterate: " << global_gradient_estimation.vectorL2Norm() << '\n';
        std::cout << '\n';
