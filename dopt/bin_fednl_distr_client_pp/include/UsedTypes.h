...
struct ServerContext
{
...
#else
    size_t roundToStart = 0;                    ///< Signal from master about round number that can be started
#endif
    ...
    */
    ...
    bool get_hessian = false;              ///< Flag for get hessian at current iterate

    bool get_local_g_direction = false;    ///< Flag for get local g direction at current iterate

    bool get_lk_difference = false;        ///< Flag for get local Lk direction at current iterate 

    bool get_client_full_gradient = false; ///< Flag for get local full gradient direction at current iterate from client

    bool client_is_active = false;         ///< Client is currently active in this compute round
    ...
    {
        sig_terminate = 10,
        sig_active_update_round_and_iterate = 11,
        sig_nonactive_update_round          = 12,

        sig_request_full_hessian_in_client = 13,
        sig_response_full_hessian_in_client = 14,

        sig_request_local_g_direction_in_client = 15,
        sig_response_local_g_direction_in_client = 16,

        sig_request_lk_difference_in_client = 17,
        sig_response_lk_difference_in_client = 18,

        sig_request_full_gradient_in_client = 19,
        sig_response_full_gradient_in_client = 20
    };
    ...
    void receiveUpdates(dopt::Socket* serverConnection)
    {
        ...
        // for (;;)
        {
            ...
            switch (ctrl_action)
            {
                ...
                case sig_active_update_round_and_iterate:
                {
                ...
                case sig_nonactive_update_round:
                {
                    serverConnection->recvData(&roundToStart, sizeof(roundToStart));
                    sz -= sizeof(roundToStart);
                ...
                case sig_request_local_g_direction_in_client:
                {
                    if (currenIterate == nullptr)
                    {
                        currenIterate = new VectorND_d(sz / sizeof(VectorND_d::TElementType));
                    ...
                    serverConnection->recvData(currenIterate->data(), sz);
                    get_local_g_direction = true;

                    break;
                }
                
                case sig_request_lk_difference_in_client:
                {
                    if (currenIterate == nullptr)
                    {
                        currenIterate = new VectorND_d(sz / sizeof(VectorND_d::TElementType));
                    ...
                    serverConnection->recvData(currenIterate->data(), sz);
                    get_lk_difference = true;

                    break;
                }
                
                case sig_request_full_gradient_in_client:
                {
                    if (currenIterate == nullptr)
                    {
                        currenIterate = new VectorND_d(sz / sizeof(VectorND_d::TElementType));
...
#if !D_OPT_PACK_TRANSFERED_MESSAGES
                buffer.putByte(ctrl);
                buffer.putUnsignedVaryingInteger(sizeof(roundToStart));
                buffer.putBytes(&roundToStart, sizeof(roundToStart));
#else
                buffer.putByte(ctrl);
                buffer.putUnsignedVaryingInteger(sizeof(roundToStart) + currenIterate->sizeInBytes());
...
struct WorkerContext
{
    ...
    enum ControlSignals : uint8_t
    {
        sig_get_worker_desciption                                       = 21,
        sig_request_matrix_vector_multiplication_with_samples           = 22,
        sig_request_matrix_vector_multiplication_with_samples_tranpose  = 23,
        sig_terminate_loop                                              = 24,
        sig_halt                                                        = 25,
...
struct WorkerContextForGD final: public WorkerContext
{
    ...
    , fiValue(VectorND_d::TElementType())
    {}
...
struct WorkerContextForFedNLControlBlock
{
    ...
    bool messageToSendClientHasBeenInitialized; ///< Signal setuped by worker when it has been initialized
#endif
};
